...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render one 1-frame LINGO debug video per scene for camera/framing inspection."
    )
    parser.add_argument(
        "--episodes-dir",
        type=Path,
        default=ROOT_DIR / "data" / "preprocessed" / "lingo" / "episodes_v2",
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=ROOT_DIR / "data" / "preprocessed" / "lingo" / "scene_debug_vis",
    ...
    parser.add_argument("--camera-scale-oblique", type=float, default=0.67)
    parser.add_argument("--camera-scale-topdown", type=float, default=1.13)
    parser.add_argument(
        "--show-targets",
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        progress = tqdm(task_specs, total=len(task_specs), desc="lingo scene debug", unit="scene")
        ...
        progress = tqdm(total=len(task_specs), desc="lingo scene debug", unit="scene")
    ...
    print(f"dispatched {total} lingo scene debug renders")
